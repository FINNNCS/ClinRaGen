...
class PatientDataset(object):
    def __init__(self, data_dir, Rationale_ts = False, inlcude_rationale = False,flag="train",):
        ...
        self.rationale_dir = "xxx"
        print(f"Rationale dir: {self.rationale_dir }")
        self.stopword = list(pd.read_csv('xxx/stopwods.csv').values.squeeze())
    ...
    def __getitem__(self, idx):
        ...
        hadm_id = self.patient_df["HADM_ID"][idx]
        subj_id = self.patient_df["SUBJECT_ID"][idx]
        # print(self.patient_df["TEXT"][idx])
        lab_rationale = json.load(open(f"{self.rationale_dir}/{self.flag}/{subj_id}_{hadm_id}_rationale.json"))["lab_rationale"]
        text_rationale = json.load(open(f"{self.rationale_dir}/{self.flag}/{subj_id}_{hadm_id}_rationale.json"))["text_rationale"]
        ...
        return numeric_data,disease_names,text,labels,lab_description,text_rationale,lab_rationale,hadm_id,subj_id
